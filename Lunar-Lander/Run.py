...
experiment_number = 0 #Change this value in order to save your videos in different folders and save your trained models with different names
...
def run_experiment(environment, agent, environment_configs, agent_configs, experiment_configs, finetune=False, PATH = 'new_results/current_model_700.pth'):
    ...
    for run in tqdm(range(experiment_configs['num_runs'])):
        ...
        for episode in tqdm(range(start_episode, start_episode + experiment_configs['num_episodes'])):
            ...
            if episode == start_episode + experiment_configs['num_episodes'] - 1:
                ...
                os.path.join('new_results', 'Experiment_{}_current_model_{}.pth'.format(experiment_number, episode+1)))
                
            print('Experiment:{}, Run:{}, episode:{}, reward:{}'.format(experiment_number, run, episode, episode_reward))
...
sum_reward = run_experiment(environment, agent, environment_configs, agent_configs, experiment_configs, finetune = False)

smoothed_reward = smooth(sum_reward, 25)


plot_reward(sum_reward, 'Reward of each episode (unsmoothed data)')

plot_reward(smoothed_reward, 'Smoothed reward')

model_path = os.path.join('new_results', 'Experiment_{}_current_model_{}.pth'.format(experiment_number, experiment_configs['num_episodes']))
...
env = wrappers.Monitor(env, './videos_experiment{}/'.format(experiment_number) + '/')
